...
def main() -> None:
    ...
    connection_manager = PostgresConnectionManager(config=config, database_url=database_url)
    vector_store = PgVectorStore(connection_manager)

    vector_store.upsert(embedded_chunks)
    print(f"Upserted {len(embedded_chunks)} chunks into report_chunks")


    query_text = 'how rough was the sea and what was the wind like'
    query_vector = embedding_service._provider.embed_batch([query_text])[0]


    results = vector_store.similarity_search(query_vector, top_k=3)
    print(f"\nTop {len(results)} results for query: '{query_text}'\n")
    for r in results:
        print(f"[{r.similarity_score:.4f}] ({r.chunk_id}) {r.content[:100]}...")
